...
uint atomicAdd1(out uint foo, uint operand) {
  return foo += operand;
}
void new_bucket(out uint bucket_offset) {
  uint count = atomicAdd1(_counter(ub.bucket_counter).val, 1);
...
uint atomicCompSwap1(out uint foo, uint cur, uint replacement) {
  uint temp = foo;
  foo = replacement;
  return temp;
}
...
bool sethash(uint value) {
  ...
  if ( existing_offset == 0)
	...
	return false;
      ...
      actual = atomicCompSwap1(_table(ub.table + index).val, 0, new_offset);
  ...
  for ( uint depth = 0; depth < max_depth; depth++ ) {
    for ( uint i = 0; i < 7; i++ ) {
      actual = atomicCompSwap1(_bucket(ub.bucket + uint64_t(existing_offset)).val[i], 0, value);
	...
	uint actual = atomicCompSwap1(_bucket(ub.bucket + uint64_t(existing_offset)).val[7], 0, new_offset);
	if ( actual == 0 ) {
	  // success
	  return true;
	}
