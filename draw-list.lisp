...
(eval-when (:compile-toplevel :load-toplevel)
  ...
  (unless krma::*debug*
    ...
    (declaim (inline %draw-list-add-filled-2d-triangle-strip))
    ...
    (declaim (inline %draw-list-draw-filled-3d-triangle-strip))
    (declaim (inline %draw-list-add-filled-3d-triangle-strip))
...
(defun %draw-list-draw-2d-circular-arc (2d-draw-list ub32-oid bool-closed? ub32-color sf-elevation
  ...
  (let* ((index-array (draw-list-index-array 2d-draw-list))
    ...
    (with-draw-list-transaction (%draw-list-draw-2d-circular-arc 2d-draw-list first-index vtx-offset)
      (let* ((dtheta (- df-end-angle df-start-angle)))
...
(defun %draw-list-add-filled-2d-triangle-list (2d-draw-list ub32-oid atom-group model-mtx ub32-color sf-elevation seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 2d-draw-list))
    ...
    (with-draw-list-transaction (%draw-list-add-filled-2d-triangle-list 2d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
...
(defun %draw-list-add-filled-2d-triangle-strip (3d-draw-list ub32-oid atom-group model-mtx ub32-color sf-elevation seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-filled-2d-triangle-strip 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
	 (progn
	   (loop for (x y) on seq-vertices by #'cddr
  ...
  (let* ((index-array (draw-list-index-array 2d-draw-list))
	...
	(list
	 (loop for (x y) on seq-vertices by #'cddr
	       ...
	       do (setq x (clampf x))
	       (setq y (clampf y))
	       (setq offset (standard-3d-vertex-array-push-extend vertex-array ub32-oid x y sf-elevation ub32-color))
	       (incf number-of-vertices)
	       when (>= i 2)
...
(defun %draw-list-add-filled-3d-triangle-strip (3d-draw-list ub32-oid atom-group model-mtx ub32-color seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-filled-3d-triangle-strip 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
	 (progn
	   (loop for (x y z) on seq-vertices by #'(lambda (list)
...
(defun %draw-list-draw-filled-3d-triangle-list-with-normals (3d-draw-list ub32-oid ub32-color seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
	 ...
	 (number-of-vertices 0))
    (declare (type fixnum vtx-offset number-of-vertices))
    (with-draw-list-transaction (%draw-list-draw-filled-3d-triangle-list-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list (loop for (x y z nx ny nz) on seq-vertices by #'(lambda (list)
								(nthcdr 6 list))
		    do (setq x (clampf x))
		       (setq y (clampf y))
		       (setq z (clampf z))
...
(defun %draw-list-add-filled-3d-triangle-list
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-filled-3d-triangle-list 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
...
(defun %draw-list-draw-filled-3d-triangle-list
    (3d-draw-list ub32-oid ub32-color seq-vertices)
  (declare (type 3d-vertex-draw-list-mixin 3d-draw-list))
  (declare (type (unsigned-byte 32) ub32-oid ub32-color))
  (declare (type sequence seq-vertices))
  ;; must be at least three vertices to succeed
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-draw-filled-3d-triangle-list 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list (loop for (x y z) on seq-vertices by #'cdddr
		    do (setq x (clampf x))
		       (setq y (clampf y))
...
(defun %draw-list-add-filled-3d-triangle-strip-with-normals
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-filled-3d-triangle-strip-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
	 (progn
...
(defun %draw-list-draw-multicolor-3d-triangle-list-with-normals (3d-draw-list ub32-oid seq-vertices)
  (declare (type 3d-vertex-draw-list-mixin 3d-draw-list))
  (declare (type sequence seq-vertices))
  ;; must be at least three vertices to succeed
  (let* ((index-array (draw-list-index-array 3d-draw-list))
	 ...
	 (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-draw-multicolor-3d-triangle-list-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
...
(defun %draw-list-add-multicolor-3d-triangle-list-with-normals
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-multicolor-3d-triangle-list-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list (loop for (x y z nx ny nz color) on seq-vertices by #'(lambda (list)
								      (nthcdr 7 list))
		    for i from 0 below #.most-positive-fixnum
...
(defun %draw-list-draw-textured-3d-triangle-list (3d-draw-list ub32-oid ub32-color seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
	 ...
	 (first-index (foreign-array-fill-pointer index-array))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-draw-textured-3d-triangle-list 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
...
(defun %draw-list-add-textured-3d-triangle-list (3d-draw-list ub32-oid atom-group model-mtx texture ub32-color seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-textured-3d-triangle-list 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
	 (loop for (x y z u v) on seq-vertices by #'(lambda (list)
						      (nthcdr 5 list))
...
(defun %draw-list-draw-textured-3d-triangle-list-with-normals (3d-draw-list ub32-oid ub32-color seq-vertices)
  ;; set the draw-list-texture!
  (declare (type 3d-vertex-draw-list-mixin 3d-draw-list))
  (declare (type (unsigned-byte 32) ub32-oid ub32-color))
  (declare (type sequence seq-vertices))
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-draw-textured-3d-triangle-list-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
...
(defun %draw-list-add-textured-3d-triangle-list-with-normals
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
         ...
         (number-of-vertices 0))
    (declare (type fixnum number-of-vertices))
    (with-draw-list-transaction (%draw-list-add-textured-3d-triangle-list-with-normals 3d-draw-list first-index vtx-offset)
      (etypecase seq-vertices
	(list
	 (loop for (x y z nx ny nz u v) on seq-vertices by #'(lambda (list)
							       (nthcdr 8 list))
...
(defun %draw-list-draw-multicolor-3d-convex-polygon-with-normals (3d-draw-list ub32-oid seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
	 ...
	 (offset -1)
	 (elem-count 0))
    (declare (type fixnum vtx-offset offset elem-count))
...
(defun %draw-list-draw-multicolor-3d-convex-polygon (3d-draw-list ub32-oid seq-vertices)
  ...
  (let* ((index-array (draw-list-index-array 3d-draw-list))
    ...
    (let ((first-index (foreign-array-fill-pointer index-array))
	  ...
	  (offset -1)
	  (elem-count 0))
      (declare (type fixnum vtx-offset elem-count offset))
...
(defun %draw-list-add-textured-sphere (3d-draw-list
  ...
  (let* ((va (draw-list-vertex-array 3d-draw-list))
         ...
         (vtx-offset (foreign-array-fill-pointer va))
         (first-index (foreign-array-fill-pointer ia)))
    (with-draw-list-transaction (%draw-list-add-textured-sphere 3d-draw-list first-index vtx-offset)
      (when (> df-radius 0.0d0)
	(let* ((sector-count fixnum-resolution)
               (stack-count (floor fixnum-resolution 2))
               (sector-step (/ 2pi sector-count))
			     ...
			     do (let* ((sector-angle (* j sector-step)))
				  ...
				  (let ((x (+ (* xy (cos sector-angle)) df-origin-x))
				    ...
				    (let ((nx (clampf (/ (- x df-origin-x) df-radius)))
				      ...
				      (textured-3d-vertex-with-normal-array-push-extend
				       va ub32-oid
				       (clampf x) (clampf y) (clampf z) nx ny nz tx ty
				       ub32-color))))
				do (setq tx (/ (float j) (1- sector-count))))))
		   do (setq ty (/ (float i) (1- stack-count))))
          (loop for i from 0
		repeat stack-count
                with k1 with k2
                do (setq k1 (* i (1+ sector-count))
