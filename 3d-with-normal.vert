...
void main () {
  world_position = pc.model * vec4(inPosition.xyz, 1.0);
  normal = (transpose(inverse(pc.model)) * vec4(inNormal, 0.0)).xyz;
  view_inv = inverse(ub.view);
  gl_Position = ub.vproj * world_position;
